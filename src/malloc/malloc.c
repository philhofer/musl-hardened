...
/* TODO(phil): handle 64k pages, etc.;
 * this limitation comes directly from
 * the hardened_malloc sources */
...
void __malloc_donate(void *p, size_t m) { return; }
...
static noreturn void fatal_error(const char *s);

static int get_metadata_key(void);
static void *memory_map(size_t size);
static int memory_map_fixed(void *ptr, size_t size);
static int memory_unmap(void *ptr, size_t size);
static int memory_protect_ro(void *ptr, size_t size);
static int memory_protect_rw(void *ptr, size_t size);
static int memory_protect_rw_metadata(void *ptr, size_t size);
static int memory_remap(void *old, size_t old_size, size_t new_size);
static int memory_remap_fixed(void *old, size_t old_size, void *new, size_t new_size);
static void memory_set_name(void *ptr, size_t size, const char *name);

static void *allocate_pages(size_t usable_size, size_t guard_size, bool unprotect, const char *name);
static void *allocate_pages_aligned(size_t usable_size, size_t alignment, size_t guard_size, const char *name);
static void deallocate_pages(void *usable, size_t usable_size, size_t guard_size);
static inline size_t hash_page(const void *p);

static void chacha_keysetup(chacha_ctx *x, const u8 *k);
static void chacha_ivsetup(chacha_ctx *x, const u8 *iv);
static void chacha_keystream_bytes(chacha_ctx *x, u8 *c, u32 bytes);
...
static void random_state_init(struct random_state *state);
static void random_state_init_from_random_state(struct random_state *state, struct random_state *source);
static void get_random_bytes(struct random_state *state, void *buf, size_t size);
static u16 get_random_u16(struct random_state *state);
static u16 get_random_u16_uniform(struct random_state *state, u16 bound);
static u64 get_random_u64(struct random_state *state);
static u64 get_random_u64_uniform(struct random_state *state, u64 bound);
...
static atomic_uint thread_arena_counter;
#else
...
static union {
    struct {
        atomic_uintptr_t slab_region_start;
...
static inline void *get_slab_region_start() {
    return (void *)atomic_load_explicit(&ro.slab_region_start, memory_order_acquire);
}
...
static inline void stats_small_allocate( struct size_class *c,  size_t size) {
#if CONFIG_STATS
    c->allocated += size;
    c->nmalloc++;
#endif
...
static inline void stats_small_deallocate( struct size_class *c,  size_t size) {
#if CONFIG_STATS
    c->allocated -= size;
    c->ndalloc++;
#endif
...
static inline void stats_slab_allocate( struct size_class *c,  size_t slab_size) {
#if CONFIG_STATS
    c->slab_allocated += slab_size;
#endif
}

static inline void stats_slab_deallocate( struct size_class *c,  size_t slab_size) {
#if CONFIG_STATS
    c->slab_allocated -= slab_size;
#endif
}
...
static struct slab_size_class_info slab_size_class(const void *p) {
    size_t offset = (const char *)p - (const char *)get_slab_region_start();
    unsigned arena = 0;
    if (N_ARENA > 1) {
...
static inline void stats_large_allocate( struct region_allocator *ra,  size_t size) {
#if CONFIG_STATS
    ra->allocated += size;
#endif
}

static inline void stats_large_deallocate( struct region_allocator *ra,  size_t size) {
#if CONFIG_STATS
    ra->allocated -= size;
#endif
}
...
static inline unsigned init(void) {
    ...
    thread_arena = arena = atomic_fetch_add_explicit(&thread_arena_counter, 1, memory_order_acq_rel) % N_ARENA;
#endif
    if (unlikely(!is_init())) {
        init_slow_path();
    }
...
__attribute__((constructor(101))) static void trigger_early_init(void) {
...
void *memalign(size_t alignment, size_t size) __attribute__((alias("aligned_alloc")));
...
void free_sized(void *p, size_t expected_size) {
    if (p == NULL) {
        return;
    }
...
size_t malloc_object_size_fast(void *p) {
    if (p == NULL) {
        return 0;
    }
...
/* NOTE(phil): apparently only used in mallopt()
...
noreturn static void fatal_error(const char *s) {
